...
username=$(curl -sS https://raw.githubusercontent.com/myridwan/izinvps/ipuk/ipx | grep $MYIP | awk '{print $2}')
valid=$(curl -sS https://raw.githubusercontent.com/myridwan/izinvps/ipuk/ipx | grep $MYIP | awk '{print $3}')
today=$(date +"%Y-%m-%d")
d1=$(date -d "$valid" +%s)
d2=$(date -d "$today" +%s)
